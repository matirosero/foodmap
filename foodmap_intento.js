...
function populate_side(country_name,country_code,search_terms) {
	...
	$("a.clickme").on("click", function(){
		...
		if (!result_name) {
			...
			new_side_header = '<h2>' + country_name + ' <span class="small">0' + result_number + ' ' + result_kind + '</span></h2>';

			if (result_class == 'ingredient') {
				new_side_filter = '<label>Filtrar por </label>\
						<select name="region">\
		...
		$("#sidepanel-header").html(new_side_header);
		$("#sidepanel-filter").html(new_side_filter);
		$("#sidepanel-content .blocks").html(new_side_content);
